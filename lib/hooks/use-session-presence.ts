...
export function useSessionPresence({
  ...
  const setMyLocation = useCallback(
    (location: UserLocation) => {
      ...
      setMembers(prev => prev.map(m => (m.id === currentUserId ? { ...m, location } : m)));
    },
  ...
  useEffect(() => {
    ...
    const handleSubscriptionSucceeded = (data: {
      ...
      if (data.members && typeof data.members === 'object') {
        Object.entries(data.members).forEach(
          ([id, info]: [string, { id: string; name: string; email: string; color: string }]) => {
            membersList.push({
              id: info.id || id,
    ...
    const handleLocationUpdate = (data: { userId: string; location: UserLocation }) => {
      ...
      setMembers(prev =>
        prev.map(m => (m.id === data.userId ? { ...m, location: data.location } : m))
      );
    };
